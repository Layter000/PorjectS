...
#include "PlayerCameraHeroComponent.h"

#include "CameraPlayerGameplayTags.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PlayerCameraHeroComponent)
...
UPlayerCameraHeroComponent::UPlayerCameraHeroComponent(const FObjectInitializer& ObjectInitializer)
...
void UPlayerCameraHeroComponent::FollowTarget(AActor* Target)
{
}

void UPlayerCameraHeroComponent::UnFollowTarget()
{
}

ALyraCharacter* UPlayerCameraHeroComponent::GetLyraCharacter()
{
	return GetPawnChecked<ALyraCharacter>();
}

void UPlayerCameraHeroComponent::OnRegister()
{
	Super::OnRegister();
}

void UPlayerCameraHeroComponent::BeginPlay()
{
	Super::BeginPlay();
}

void UPlayerCameraHeroComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UPlayerCameraHeroComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

void UPlayerCameraHeroComponent::InitializePlayerInput(UInputComponent* PlayerInputComponent)
{
...
void UPlayerCameraHeroComponent::Input_ZoomCamera(const FInputActionValue& InputActionValue)
{
	...
	if (Controller)
	{
		const float Value = InputActionValue.GetMagnitude();

		
	}
}

void UPlayerCameraHeroComponent::Input_EdgeScrollCamera(const FInputActionValue& InputActionValue)
{
	ConditionallyPerformEdgeScrolling();
}


void UPlayerCameraHeroComponent::Input_RotateCameraLeft(const FInputActionValue& InputActionValue)
{
...
void UPlayerCameraHeroComponent::Input_RotateCameraRight(const FInputActionValue& InputActionValue)
{
	FGameplayTagContainer CameraTag;
...
void UPlayerCameraHeroComponent::QueueCameraMovementCommand(const FVector Direction, const double Scale,
	FGameplayTagContainer CameraMovementTags)
{
	CameraMovementTags.AddTagFast(TAG_Camera_Move);
	FCameraMovementCommand MovementCommand;
	MovementCommand.Direction = Direction;
	MovementCommand.Scale = Scale;
	MovementCommand.CameraTag = CameraMovementTags;
	CameraMovementCommandQueue.Enqueue(MovementCommand);
}

void UPlayerCameraHeroComponent::QueueCameraRotationCommand(const float Rotation,
	FGameplayTagContainer CameraMovementTags)
{
	CameraMovementTags.AddTagFast(TAG_Camera_Rotate);
	FCameraMovementCommand MovementCommand;
...
void UPlayerCameraHeroComponent::ApplyMoveCameraCommands()
{
	APawn* Pawn = GetPawn<APawn>();
...
void UPlayerCameraHeroComponent::ConditionallyEnableEdgeScrolling() const
{
...
void UPlayerCameraHeroComponent::ConditionallyPerformEdgeScrolling()
{
...
void UPlayerCameraHeroComponent::EdgeScrollLeft()
{
	APawn* Pawn = GetPawn<APawn>();
...
void UPlayerCameraHeroComponent::EdgeScrollRight()
{
	APawn* Pawn = GetPawn<APawn>();
	const auto MousePosition = UWidgetLayoutLibrary::GetMousePositionOnViewport(GetWorld());
	const auto ViewportSize = UWidgetLayoutLibrary::GetViewportWidgetGeometry(GetWorld()).GetLocalSize();
...
void UPlayerCameraHeroComponent::EdgeScrollUp()
{
	APawn* Pawn = GetPawn<APawn>();
	const auto MousePosition = UWidgetLayoutLibrary::GetMousePositionOnViewport(GetWorld());
	const auto ViewportSize = UWidgetLayoutLibrary::GetViewportWidgetGeometry(GetWorld()).GetLocalSize();
...
void UPlayerCameraHeroComponent::EdgeScrollDown()
{
	APawn* Pawn = GetPawn<APawn>();
	const auto MousePosition = UWidgetLayoutLibrary::GetMousePositionOnViewport(GetWorld());
	const auto ViewportSize = UWidgetLayoutLibrary::GetViewportWidgetGeometry(GetWorld()).GetLocalSize();
...
void UPlayerCameraHeroComponent::BindInputTags(ULyraInputComponent* PlayerInputComponent,
	const ULyraInputConfig* InputConfig)
{
	const FCameraPlayerGameplayTags& GameplayTags = FCameraPlayerGameplayTags::Get();
	...
	PlayerInputComponent->BindNativeAction(InputConfig, GameplayTags.Input_CameraZoom, ETriggerEvent::Triggered, this, &ThisClass::Input_ZoomCamera, /*bLogIfNotFound=*/ false);
